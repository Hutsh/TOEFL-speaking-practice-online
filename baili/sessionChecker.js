/**
 * bxsnet ajax session request
 */
var BXSSessionChecker = {
	interval: 5,//minutes
	connectUrl:"../user/sessionChecker.do?method=req",//request action
	intervaling: function(){return BXSSessionChecker.interval*60*1000;},
	counter: 0,
	start:function(intervalTime){
		BXSSessionChecker.title = window.document.title;
		if(intervalTime) BXSSessionChecker.interval = intervalTime;
		window.document.title =BXSSessionChecker.title+ ".";
		BXSSessionChecker.timerId = window.setInterval(function(){
			BXSSessionChecker.req = new AJAXRequest();
			BXSSessionChecker.req.get(BXSSessionChecker.connectUrl+BXSSessionChecker.getData(),function(res){
			...
			BXSSessionChecker.req.onexception = function(){
				window.document.title = BXSSessionChecker.title+ "...";
				alert("网络异常中断!");
			}
		},BXSSessionChecker.intervaling());
	},
	clear:function(){
		window.clearInterval(BXSSessionChecker.timerId);
	},
	getData:function(){
